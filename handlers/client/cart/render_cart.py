...
async def render_cart(
    ...
    # Гарантируем загрузку связанной корзины и заказов
    ...
    if not user or not user.cart:
        ...
        main_kb = kb_manager.get_main_kb(orders=orders_count, cart=0)
    ...
    subtotal = sum(item.quantity * float(item.product.price) for item in user.cart if item.product)
